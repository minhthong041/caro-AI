...
│   └── reports/          # Lịch sử train CNN sau khi huấn luyện
├── src/
...
```txt
data/reports/cnn_training_history.json
data/reports/cnn_training_history.csv
data/reports/cnn_training_history.png
```

File PNG chỉ được tạo nếu môi trường có `matplotlib`. Nếu không có, JSON/CSV vẫn đủ để đưa số liệu loss, accuracy, top-3 accuracy và validation vào báo cáo.

Các log đã dùng để train sẽ được chuyển sang:
...
Trước khi public hoặc release, nên loại bỏ các file sinh tự động như `__pycache__/`, log tạm thời hoặc model lớn nếu không muốn đưa chúng vào repository.
